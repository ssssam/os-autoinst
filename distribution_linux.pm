...
=head1 DISTRIBUTION_LINUx

=head1 SYNOPSIS

Base distribution with common settings for Linux operating systems.
=cut

package distribution_linux;
...
Set up the consoles that Linux systems commonly have available.

Graphical consoles 'vt1' through 'vt9' correspond to the virtual consoles
set up by the kernel.

Text console 'serial' corresponds to the first virtio serial console, if
running with QEMU backend. (For other backends, there will be no 'serial'
console created so you can set it up yourself).

When accessing the serial console, you can use `serial_terminal::login()`
to wait for a login prompt and enter user credentials to reach a shell prompt.

See also: "multi console support" in the testapi documentation, and
"Advanced Features" in the openQA test developer guide,
=cut

sub init_consoles() {
    for my $n (1..9) {
        $self->add_console('vt$n', 'tty-console', {tty => $n});
    }

    # Set up serial console
    if (check_var('BACKEND', 'qemu')) {
        $self->add_console('serial', 'virtio-terminal', {});
    }
        # FIXME: this is really backend config or test-specific config.
